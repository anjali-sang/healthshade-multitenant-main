...
{
    public string $tableName = 'user-list-vv6gjo-table';
    ...
    {
        return User::with('location')->where('users.is_active',true)->where('users.organization_id',auth()->user()->organization_id);
    ...
    public function fields(): PowerGridFields
    {
        return PowerGrid::fields()
            ...
            ->add('is_deleted')
            ->add('role_name', function ($item) {
                return $item->role_id == '2' ? 'Admin' : $item?->role?->role_name;
            })
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make('Is deleted', 'is_deleted')
                ...
                Column::make('Role', 'role_name') // Use role_name instead of role_id
                ->sortable()
                ->searchable(),
                ...
                Column::add()
                ->title('Location')
                ->field('location_name', 'locations.name')
                ->sortable()
                ->searchable(),
    ...
    public function actions(User $row): array
{
    return [
        Button::add('edit')
        ->slot('
            <span class="min-w-12" x-data="{ loading: false }" x-on:click="loading = true; $dispatch(\'edit-user\', { rowId: ' . $row->id . ' }); setTimeout(() => loading = false, 1000)">
